...
// Clearly significant: low12 (t=3.8), lastDigitDup (t=2.6). The others are weak (|t| < 1.7).
...
export class LottoEngine {
     ...
     */
    generate({ candidates = 30, rng = Math.random } = {}) {
        let best = null;
        let accepted = 0;
        for (let guard = 0; accepted < candidates && guard < candidates * 20; guard++) {
            const numbers = randomCombo(rng);
        ...
        return { ...best, analysis: this.analyze(best.numbers) };
    }
    ...
    analyze(numbers) {
        ...
        const lines = [
            ...
            `⚖️ 당첨 확률은 어떤 조합이든 똑같이 1/${TOTAL_COMBOS.toLocaleString('ko-KR')}`,
        ];
    ...
    getFrequency(weeks = 52) {
        const counts = Object.fromEntries(Array.from({ length: 45 }, (_, i) => [i + 1, 0]));
